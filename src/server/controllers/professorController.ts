...
export const listarAulasDoProfessor = async (req: Request, res: Response) => {
  try {
    ...
    // Adaptar formato para frontend (recalculando vagas ocupadas pelas reservas ATIVAS)
    const aulasFormatadas = aulas.map(aula => {
      const reservas = aula.reservas || [];
      const ocupadasAtivas = reservas.filter(r => String((r as any)?.status || '').toLowerCase() === 'ativa').length;
      const vagas_ocupadas = ocupadasAtivas;
      return ({
        id: aula.id,
        titulo: aula.titulo,
