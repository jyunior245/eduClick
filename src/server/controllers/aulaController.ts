...
export const criarAula = async (req: Request, res: Response) => {
  try {
    ...
    // Aceitar tanto chaves do frontend (dataHora, maxAlunos) quanto do backend (data_hora, vagas_total)
    const {
      titulo,
      conteudo,
      valor,
      duracao,
      status,
      data_hora: dataHoraSnake,
      dataHora: dataHoraCamel,
      vagas_total: vagasSnake,
      maxAlunos: vagasCamel
    } = req.body as any;

    if (!titulo) return res.status(400).json({ error: 'titulo obrigatório.' });

    const vagasStr = vagasSnake ?? vagasCamel;
    const vagas_total = typeof vagasStr === 'string' ? parseInt(vagasStr, 10) : Number(vagasStr);
    if (Number.isNaN(vagas_total) || !Number.isInteger(vagas_total) || vagas_total <= 0) {
      return res.status(400).json({ error: 'vagas_total/maxAlunos obrigatório e deve ser inteiro positivo.' });
    }

    const dataStr = dataHoraSnake ?? dataHoraCamel;
    if (!dataStr) return res.status(400).json({ error: 'data_hora/dataHora obrigatório.' });
    const dataHoraValida = !isNaN(Date.parse(dataStr));
    if (!dataHoraValida) return res.status(400).json({ error: 'data_hora/dataHora inválido.' });
    ...
    const aula = aulaRepo.create({
      ...
      titulo,
      vagas_total,
      data_hora: new Date(dataStr),
      conteudo,
      valor,
      duracao,
      // se vier do cliente, respeita; caso contrário, @BeforeInsert define default
      ...(status ? { status } : {})
    });
...
export const reagendarAulaServidor = async (req: Request, res: Response) => {
  try {
    ...
    const aula = await aulaRepo.findOne({
      where: { id },
      relations: ['reservas', 'reservas.aluno', 'reservas.aluno.usuario']
    });
    ...
    // Notificar alunos com reserva ativa sobre reagendamento (usa serviço centralizado)
    try {
      await NotificationService.notificarAulaReagendada(aula.id, aula.data_hora as any);
    } catch (e) {
      console.error('[REAGENDAR AULA] erro ao notificar alunos:', e);
    }
...
export const listarMinhasAulas = async (req: Request, res: Response) => {
  try {
    ...
    const aulasFormatadas = aulas.map(aula => {
      // Calcular ocupação a partir de reservas ativas para evitar inconsistências
      const reservasAtivas = (aula.reservas || []).filter(r => String((r as any)?.status || '').toLowerCase() === 'ativa');
      const vagasOcupadas = reservasAtivas.length;
...
export const cancelarReserva = async (req: Request, res: Response) => {
  try {
    ...
    const aula = await aulaRepository.findOne({
      ...
      relations: ['professor', 'professor.usuario', 'reservas', 'reservas.aluno', 'reservas.aluno.usuario']
    });
    ...
    // Cancelar a reserva (marcar como cancelada) e persistir a entidade Reserva
    const reserva = aula.reservas[reservaIndex];
    reserva.status = 'cancelada';
    reserva.data_cancelamento = new Date();

    // Persistir a reserva cancelada
    const reservaRepository = AppDataSource.getRepository(Reserva);
    await reservaRepository.save(reserva);

    // Recalcular vagas ocupadas com base nas reservas ativas atuais
    const ativas = (aula.reservas || []).filter(r => String((r as any)?.status || '').toLowerCase() === 'ativa').length;
    aula.vagas_ocupadas = Math.max(0, ativas);
    ...
    // Notificar professor sobre cancelamento da reserva
    try {
      const profUid = (aula.professor as any)?.usuario?.uid;
      if (profUid) {
        await NotificationService.sendToUsuarioUid(
          profUid,
          'Reserva cancelada',
          `${nome} cancelou a reserva da aula "${aula.titulo}"`,
          { aulaId: String(aula.id) }
        );
      }
    } catch (e) {
      console.error('[cancelarReserva] erro ao notificar professor:', e);
    }
