...
export class TypeORMAulaService {
  ...
  async reservarAula(aulaId: string, nome: string, telefone: string, email: string, alunoFcmToken?: string | null): Promise<boolean> {
    ...
    try {
      ...
      // 2.1. Se recebemos alunoFcmToken, também persistir no Usuario para permitir push por UID no futuro
      if (alunoFcmToken && typeof alunoFcmToken === 'string' && alunoFcmToken.trim()) {
        try {
          if (!usuario.fcmToken || usuario.fcmToken !== alunoFcmToken) {
            usuario.fcmToken = alunoFcmToken;
            await queryRunner.manager.getRepository(Usuario).save(usuario);
          }
        } catch (e) {
          console.warn('[reservarAula] falha ao salvar fcmToken no Usuario:', e);
        }
      }
