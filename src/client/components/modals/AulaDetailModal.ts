...
export class AulaDetailModal {
  static render(aula: AulaDetailData): string {
    ...
    const vagas = `${ocupadas} / ${aula.maxAlunos}`;
    const vagasRestantes = Math.max(0, aula.maxAlunos - ocupadas);
    const content = `
      <div class="row">
        <div class="col-md-6">
          <div class="mb-3">
      ...
      <div class="mb-3">
        <label class="form-label fw-bold">Reservas (${ocupadas})</label>
        <div class="border rounded p-3" style="max-height: 200px; overflow-y: auto;">
          ${ocupadas > 0 ? 
            reservasAtivas.map(reserva => this.renderReserva(reserva)).join('') :
            '<div class="text-muted">Nenhuma reserva ativa.</div>'
          }
        </div>
      </div>
    `;
    ...
    return `
      ...
      <div class="row">
        <div class="col-md-6">
          <div class="mb-3">
            <label class="form-label fw-bold">Vagas</label>
            <div class="form-control-plaintext">${ocupadas} / ${aula.maxAlunos}</div>
          </div>
        </div>
        <div class="col-md-6">
          <div class="mb-3">
