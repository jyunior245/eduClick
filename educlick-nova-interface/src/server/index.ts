...
AppDataSource.initialize().then(() => {
  ...
  app.listen(port, () => {
    ...
    // Scheduler simples: a cada minuto, envia lembretes de 30 minutos antes da aula
    setInterval(async () => {
      try {
        if (!AppDataSource.isInitialized) return;
        await NotificationService.enviarLembretesAulas();
      } catch (e) {
        console.error('[Scheduler] erro ao enviar lembretes:', e);
      }
    }, 60 * 1000);
  });
}).catch((error) => {
