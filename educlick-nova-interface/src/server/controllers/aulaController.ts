...
export const reagendarAulaServidor = async (req: Request, res: Response) => {
  try {
    ...
    // Notificar alunos com reserva ativa sobre reagendamento
    try {
      const reservasAtivas = (aula.reservas || []).filter(r => String((r as any)?.status || '').toLowerCase() === 'ativa');
      console.log('[REAGENDAR AULA] reservas ativas para notificar', { aulaId: aula.id, count: reservasAtivas.length });
      for (const r of reservasAtivas) {
        const token = (r as any)?.fcmToken || (r as any)?.aluno?.usuario?.fcmToken;
        const uid = r.aluno?.usuario?.uid as string | undefined;
        if (token) {
          console.log('[REAGENDAR AULA] notificando aluno por FCM token', { reservaId: r.id });
          await NotificationService.sendToFcmToken(
            token,
            'Aula reagendada',
            `Sua aula "${aula.titulo}" foi reagendada para ${aula.data_hora}`,
            { aulaId: String(aula.id) }
          );
        } else if (uid) {
          console.log('[REAGENDAR AULA] notificando aluno por UID', { reservaId: r.id, uid });
          ...
          console.warn('[REAGENDAR AULA] reserva sem UID e sem FCM token, não foi possível notificar', { reservaId: r.id });
        }
      }
    } catch (e) {
...
export const cancelarReserva = async (req: Request, res: Response) => {
  try {
    ...
    // Atualizar vagas ocupadas (robusto mesmo se undefined)
    const ocupadasAntes = typeof aula.vagas_ocupadas === 'number' ? aula.vagas_ocupadas : ((aula.reservas || []).filter(r => String((r as any)?.status || '').toLowerCase() === 'ativa').length);
    aula.vagas_ocupadas = Math.max(0, ocupadasAntes - 1);
