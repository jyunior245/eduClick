"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.removerReserva = exports.cancelarReserva = exports.buscarReserva = exports.listarReservas = exports.criarReserva = void 0;
const criarReserva = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    res.json({ message: 'Reserva criada!' });
});
exports.criarReserva = criarReserva;
const listarReservas = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    res.json([]);
});
exports.listarReservas = listarReservas;
const buscarReserva = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    res.json({});
});
exports.buscarReserva = buscarReserva;
const cancelarReserva = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    res.json({ message: 'Reserva cancelada!' });
});
exports.cancelarReserva = cancelarReserva;
const removerReserva = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    res.json({ message: 'Reserva removida!' });
});
exports.removerReserva = removerReserva;
